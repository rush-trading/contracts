...
/**
 * @title RushRouterAlphaSponsored
 * @notice A periphery contract that acts as a router for Rush protocol operations with gated access to RushLauncher
 * functions and fee sponsorship for token launches.
 */
contract RushRouterAlphaSponsored is Nonces {
        ...
        payable
    {
        ...
        RUSH_LAUNCHER.launch{
            value: 
            // Deployment fee is sourced from the sponsor.
            _sourceLiquidityFee({ liquidityAmount: liquidityAmount, liquidityDuration: liquidityDuration })
    ...
    /**
     * @dev Source the deployment fee from the sponsor.
     *
     * Requirements:
     * - The sponsor must have approved the contract to spend the fee amount.
     *
     * @param liquidityAmount The amount of WETH liquidity to deploy.
     * @param liquidityDuration The duration of the liquidity deployment.
     */
    function _sourceLiquidityFee(
        uint256 liquidityAmount,
        uint256 liquidityDuration
    )
        internal
