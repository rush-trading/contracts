...
contract LauncherPoolInteraction is Fork_Test {
    ...
    function deployLauncher() internal {
        rushLauncher = IRushLauncher(new RushLauncher({
    ...
    function test_launch() public {
        ...
        bytes memory _data = abi.encode(_owner, defaults.ERC20_TAXABLE_RATE_BPS(), address(liquidityDeployer), users.router);
        deposit(address(weth), 10 ether);
        RL.LaunchParams memory launchParams = RL.LaunchParams(
            _kind,
            "TestTaxToken",
            "TTT",
            defaults.RUSH_ERC20_SUPPLY(),
            _data,
            1 ether,
            1000
        );
